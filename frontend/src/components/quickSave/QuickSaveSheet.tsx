...
export function QuickSaveSheet({ open, onClose }: QuickSaveSheetProps) {
  const navigate = useNavigate()
  const presets = useQuickSaveStore((s) => s.presets)
  const lastSave = useQuickSaveStore((s) => s.lastSave)
  const addSession = useQuickSaveStore((s) => s.addSession)
  ...
  const generateUpiMutation = useMutation({
    mutationFn: (data: { amount: number }) => upiService.generate({ ...data, profile: 'deposit' }),
    onSuccess: (data) => {
      setUpiData(data)
      const id = crypto.randomUUID()
      setSessionId(id)
      setPendingDeposit({ amount: data.amount })
      addSession({
        id,
        amount: data.amount,
        goalId: null,
        note: null,
        upiUrl: data.upi_url,
        paymentApp: upiSettings?.preferred_upi_app ?? null,
  ...
  const openPreferredApp = useCallback((data: UpiGenerateResponse) => {
    ...
    setAppLaunchTime(Date.now())
    setAppLaunched(true)
    setStep('upi')
  }, [upiSettings, setAppLaunchTime])
  ...
  function startQuickSave(amount: number) {
    if (!upiSettings?.upi_id) {
      setError('Please configure your UPI ID in Settings first.')
      return
    }
  ...
  function handleSaveAgain() {
    if (!lastSave) return
    setError(null)
    startQuickSave(lastSave.amount)
  }
  ...
  const displayPresets = presets.filter((p) => p.id !== 'qs-custom')
              ...
              {step === 'presets' && (
                <>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {displayPresets.map((preset) => (
                        <motion.button
                          ...
                          className={cn(
                            'flex flex-col items-center justify-center gap-1 rounded-xl py-4 px-2 font-semibold transition-all',
                            'bg-gradient-to-br from-emerald-500 to-emerald-700 text-white',
                            'shadow-md shadow-emerald-500/25 hover:shadow-emerald-500/40',
                          )}
                        >
                          <span className="text-lg">{preset.label}</span>
                          <span className="text-[10px] opacity-80 font-normal">Quick Save</span>
                        </motion.button>
                      ))}
                      <motion.button
                        whileTap={{ scale: 0.93 }}
                      ...
                      >
                        ...
                        <span className="text-[10px] opacity-80 font-normal">Enter Amount</span>
                      </motion.button>
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    ...
                    <button
                      onClick={() => { handleClose(); navigate('/add') }}
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="block w-full rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-3xl font-bold text-center py-4 tracking-tight focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500"
                    />
                  </div>
                  {error && (
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      Quick Save ₹{Number(customAmount).toLocaleString('en-IN') || '0'}
                    </Button>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4">
                  {appLaunched && (
                    <div className="flex items-center gap-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 px-4 py-3 text-sm text-emerald-700 dark:text-emerald-300">
                      ...
                      <span>Waiting for payment... Complete it in your UPI app, then come back.</span>
                    </div>
                  )}
                  <UPIQrCode
